...
tinyxml2::XMLElement* WriteChildWithChildAndAtt(const char* nodeName, const char* childName, const char* attName, const char* attValue, tinyxml2::XMLNode* root)
{
	tinyxml2::XMLElement *node = new tinyxml2::XMLElement(nodeName);
	tinyxml2::XMLElement *child = new tinyxml2::XMLElement(childName);
...
tinyxml2::XMLElement* WriteChildWithChildAndAtt(tinyxml2::XMLElement* node, const char* childName, const char* attName, const char* attValue)
{
	tinyxml2::XMLElement *child = new tinyxml2::XMLElement(childName);
	child->SetAttribute(attName, attValue);
	return node;
...
gcWString GameExplorerInfo::generateXml()
{
	...
	tinyxml2::XMLElement *playTask = doc.NewElement("Play");
		...
		if (vExeList.size() > 1)
		{
			for (size_t x=0; x<vExeList.size(); x++)
			{
				...
				tinyxml2::XMLElement *changeLog = WriteChildWithChildAndAtt("Task", "URLTask", "Link", link.c_str(), playTask);
		...
		{
			tinyxml2::XMLElement *changeLog = WriteChildWithChildAndAtt("Task", "URLTask", "Link", szChangeLogLink.c_str(), playTask);
			changeLog->SetAttribute("index", i);
		...
		tinyxml2::XMLElement *profile = WriteChildWithChildAndAtt("Task", "URLTask", "Link", szProfileLink.c_str(), playTask);
		...
		for (uint32 x=0; x<m_pItemInfo->getBranchCount(); x++)
		{
			...
			tinyxml2::XMLElement *branch = WriteChildWithChildAndAtt("Task", "URLTask", "Link", link.c_str(), playTask);
	...
	tinyxml2::XMLElement *verify = WriteChildWithChildAndAtt("Task", "URLTask", "Link", szVerifyLink.c_str(), supportTask);
		verify->SetAttribute("index", 0);
		verify->SetAttribute("name", "Verify Files");

		tinyxml2::XMLElement *uninstall = WriteChildWithChildAndAtt("Task", "URLTask", "Link", szUninstallLink.c_str(), supportTask);
