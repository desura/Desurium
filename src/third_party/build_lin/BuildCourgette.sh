...
function makeCourgette()
{
	make courgette_desura BUILDTYPE=$type -j $MAKE_CONCURRENCY 2>>$logFile 1>>$logFile
	makeReturn=$?
...
function main()
{
	...
	mv out/$type/lib.target/libcourgette_desura.so ../../../../build_out/$out/lib/ 2>>$logFile 1>>$logFile
